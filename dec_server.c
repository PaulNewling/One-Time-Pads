...
*   and receives from the client a handshake, cipherText file and key file. If the handshake
*   is correct the server then returns the decrypted plaintext of the cipherText.
*/
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        //  and return the decrypted text. This will allow the parent to continue to fork off children to 
        //  decrypt text while the parent accepts socket connections.
        spawnPid = fork();
        ...
        switch(spawnPid){
            ...
            // If the fork returns 0, this is the child process where we will read, decrypt
            ...
            case 0:
                ...
                // The full message contains the handshake, CipherText message and key, all seperated
                ...
                // First token is check against the decryption hand shake, if it is missing this connection is
                ...
                // CipherText is decrypted with the key and becomes the PlainText
                decrypt(cipherText, plainText, key);

                // PlainText is sent back to the client
                sendMsg(connectionSocket, plainText);
