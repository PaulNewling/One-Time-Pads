...
*   This program was modified to check the appropriate key and plaintext files
...
*   plaintext and keyfile are all loaded into one message and sent to the server.
*   if the handshake is accepted a cipherText is returned and the client prints
*   this text.
*/
int main(int argc, char *argv[]) {
    int socketFD, charsRead, textLength, keyLength, authenticate = 0;
    struct sockaddr_in serverAddress;
    char buffer[256], fullMessage[200000], cipherText[100000];
    ...
    // Check the plaintext and key file to ensure they are valid. Then place their lengths
    ...
    // Ensure that the key file is larger than the plaintext, this is required for a one-time pad
    if (keyLength < textLength){
        error("CLIENT ERROR: Key file length too short.\n", 1);
        
    }

    // Add the corresponding handshake message to the full message
    char authToken[5] = "enc@";
    memset(fullMessage, '\0', sizeof(fullMessage));
    strcat(fullMessage, authToken);

    // Add the plaintext and key to the full message
    ...
    // Receive the cipherText back from the server
    recvMsg(socketFD, cipherText);
